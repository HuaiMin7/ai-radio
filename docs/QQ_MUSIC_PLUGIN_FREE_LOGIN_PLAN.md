...
> 文档版本：v1.2  
> 日期：2026-09-02  
> 状态：PR #17 已合并并部署；本地真实账号播放已验证，生产跨设备扫码和完整浏览器矩阵待继续验收  
...
- 未完成：生产二维码的用户扫码确认、登录后账号数据恢复和真实歌曲播放仍需一次人工端到端验收；Chrome、Edge、Safari、Firefox、Android Chrome 和 iPhone Safari 的完整矩阵仍待执行。
...
同一部手机无法直接扫描自己屏幕中的二维码。第一版保留原始二维码图片，支持浏览器原生长按保存，并明确提示可换另一台设备扫码；页面在后台时暂停轮询，重新进入前台后恢复查询。
...
生产发布已完成服务健康、二维码会话边界、公开 Cookie 导入防护、Qwen TTS 和页面可视检查。生产真实账号扫码、数据恢复和歌曲播放仍是独立验收项；完成前不得把“线上已启用”扩大描述为“全终端验收通过”。
