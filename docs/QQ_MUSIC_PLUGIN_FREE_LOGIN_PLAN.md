...
> 文档版本：v1.1  
> 日期：2026-09-02  
> 状态：已按评审修订并完成分支开发，待真实 QQ 账号与跨设备验收  
...
- 手动 Cookie 表单仅在本地开发构建展示；公开站保留同一兼容接口供用户主动触发的 Bridge 兜底使用，不提供普通用户手动粘贴入口。
...
当前分支只完成代码与本地验证，不自动部署。生产灰度需要单独确认；如需运行时功能开关，应在部署任务中另行加入并验证，不能把“代码已完成”写成“线上已启用”。
